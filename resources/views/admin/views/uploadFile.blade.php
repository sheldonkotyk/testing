...
@section('headerscripts')
	...
	<script>
		...
		$("div#file_upload").dropzone({ 
			url: "{!! URL::to('admin/upload_file', array($type, $id)) !!}",
			maxFilesize: 2,
			dictDefaultMessage: '',
	        complete: function()
	        {
		        $.ajax({
					url: "{!! URL::to('admin/files_table', array($type, $id)) !!}",
					data: {},
					cache: 'false',
					dataType: 'html',
					type: 'get',
...
@endif
{!! $name !!} <small><span class="icon ion-upload"></span> <em>Upload Files</em> </small>
</h1>
...
@endif
			...
			<div id="panel1" class="panel panel-default magic-element width-full">
	            <div class="panel-heading">
		            <div class="panel-actions">
	                    	<div class="label label-danger offline" style="display:none;">Internet Not Connected</div>
	                    	<div class="label label-info loading" style="display:none;">Uploading</div>
	...
	@if($upload->useBox()&&$box_access_token!=''&&$upload->isBoxLoggedIn())
	...
	<div  class="panel panel-default magic-element width-full" id= "files">
	...
	$("#uploader").plupload({
		...
		preinit : {
            ...
            UploadFile: function(up, file) {
            	...
            	if("{!!$upload->useBox()!!}" == "1" && file.type =='image/jpeg') //This is for uploading images to BOX
				{
		...
		                   }
			    ...
			    else if("{!!$upload->useBox()!!}" == "1") //This is for uploading everything but images to BOX!
			    {
			    	//Send the Full size image to box.
        ...
        UploadComplete: function(up, files) {
        	...
        	$.each(files, function(i, file) {
        		...
        		if(file.status==5 && file.uploaded!='done'){
        			...
        			s3Success++;
        			var uploading='Uploading, please wait...<br/>';
					if("{!!$upload->useBox()!!}" == "1"&&boxSuccess==numFiles&&s3Success==numFiles)
					{
						uploading='Upload Complete. Scroll down to view your files.<br/>';
						window.onbeforeunload = null;
					...
					if("{!!$upload->useBox()!!}" != "1"&&s3Success==numFiles)
					{
						uploading='Upload Complete. Scroll down to view your files.<br/>';
						window.onbeforeunload = null;
						$loading.fadeOut(function () {
					...
					var boxMessage= '';
					if("{!!$upload->useBox()!!}" == "1"&&boxSuccess==numFiles)
						boxMessage = boxSuccess + ' of '+ numFiles +' files were Successfully uploaded to Box.com<br/>';
